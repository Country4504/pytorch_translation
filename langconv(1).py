...
class Node(object):
    def __init__(self,from_word, to_word=None,is_tail=True,have_child =False):
        ...
        if to_word is None:
            ...
            self.to_word = from_word or to_word
            self.data = (is_tail, have_child, from_word)
    ...
    def __str__(self):
        return '<Node ,%s,%s,%s,%s>' %(repr(self.from_word),repr(self.to_word),
                                       self.is_tail,self.hava_child)
...
class Converter(object):
    ...
    def feed(self, char):
        ...
        self.machines = [fsm for fsm in self.machine if fsm.state != FAIL]
        all_ok = True
        for fsm in self.machines:
            if fsm.state != END:
                all_ok = False
...
def StatesMachineException(Exception):
    pass
    ...
    def feed(self,char,map):
        ...
        if cond == ERROR:
            ...
            raise StatesMachineException('Translate States Machine'
                                         'have error with input data' % node)
...
register('zh-hant',zh2Hant)
register('zh-hans',zh2Hans)
